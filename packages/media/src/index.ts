...
export async function probeMediaBuffer(bytes: Uint8Array, filename: string, expectedKind: MediaKind): Promise<MediaProbe> {
  const directory = await mkdtemp(join(tmpdir(), "droploop-media-probe-"));
  const safeName = basename(filename) || `media-${expectedKind}`;
  const filePath = join(directory, safeName);
  try {
    await writeFile(filePath, bytes, { flag: "wx" });
    return await probeMediaFile(filePath, expectedKind);
  } finally {
...
export function parseProbeOutput(output: string, expectedKind: MediaKind): MediaProbe {
  ...
  const durationSeconds =
    ...
    positiveNumber(audioStream?.duration) ??
